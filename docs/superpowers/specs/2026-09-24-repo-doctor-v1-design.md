...
3. `graph`：把局部模块名映射至文件（兼容常见 `src/` 布局），解析相对/绝对导入并标注来源行；只为可静态定位的局部调用建立调用边。对于局部变量，仅当简单构造赋值或 `with ... as` 绑定能唯一定位到一个本地类时，才解析该变量的方法调用；歧义类型或其他改写仍保留为 unresolved。检测局部模块导入环。
...
- 局部实例方法调用仅支持唯一的简单本地类构造绑定，例如 `client = Client()` 或 `with modules.Client() as client`；复杂工厂、属性赋值、实例重绑定和外部类保持未解析。
