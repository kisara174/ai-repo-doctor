...
3. `graph`：把局部模块名映射至文件（兼容常见 `src/` 布局），解析相对/绝对导入并标注来源行；只为可静态定位的局部调用建立调用边。对于局部变量，仅当调用前有且只有一次无条件的简单构造赋值，或 `with ... as` 的对应 `__enter__` / `__aenter__` 能静态确认返回 `self`、方法种类与同步/异步协议匹配、签名没有额外必填参数且类上存在匹配的 `__exit__` / `__aexit__`，才解析该变量的方法调用；歧义类型、重绑定、遮蔽和条件赋值仍保留为 unresolved。检测局部模块导入环。
...
- 局部实例方法调用仅支持调用前唯一且无条件的简单本地类构造绑定，例如 `client = Client()`；上下文管理器绑定还要求对应类型的 `__enter__` / `__aenter__` 直接 `return self`，同步/异步方法类型正确、签名没有额外必填参数，且定义了匹配的 `__exit__` / `__aexit__`。复杂工厂、属性赋值、实例重绑定和外部类保持未解析。
