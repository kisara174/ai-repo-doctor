...
- [x] **Step 1: Add a parser test for decorators, recognized overload aliases, and conditional imports**
...
- [x] **Step 3: Add the model records and parser extraction**
...
- [x] **Step 4: Run parser tests and full current suite**
...
- [x] **Step 5: Commit the parser metadata slice**
...
- [x] **Step 1: Add tests for one implementation, overload-only, and multiple implementations**
...
- [x] **Step 2: Run the focused overload tests and confirm failure**
...
- [x] **Step 3: Group parsed definitions before inserting symbols**
...
- [x] **Step 4: Run graph tests and the complete suite**
...
- [x] **Step 5: Commit overload grouping**
...
- [x] **Step 1: Add a fixture covering a two-hop package re-export and both call forms**
...
- [x] **Step 2: Run the new re-export test and confirm it fails**
...
- [x] **Step 3: Build a cycle-safe export table and resolve calls through it**
...
- [x] **Step 4: Add negative tests for cycles, conflicts, and unsupported forms**
...
- [x] **Step 5: Run focused and full graph suites**
...
- [x] **Step 6: Commit re-export resolution**
...
- [x] **Step 1: Add tests for direct imports, module aliases, and subgroups**
...
- [x] **Step 2: Run the focused test and confirm there are no registration edges yet**

Run: `python3 -m unittest tests.test_semantics -v`

Expected: FAIL because no semantic pass currently exists.

- [x] **Step 3: Implement import identity and group-registration resolution**
...
- [x] **Step 4: Add false-positive tests**
...
- [x] **Step 5: Run semantic and full test suites**
...
- [x] **Step 6: Commit Click relationship recognition**
...
- [x] **Step 1: Add CLI/context tests for separate semantic outputs and schema 2**
...
- [x] **Step 2: Run the focused tests and confirm schema/relationship assertions fail**
...
- [x] **Step 3: Serialize typed records and add labeled context/impact data**
...
- [x] **Step 4: Update human-readable CLI text and README**
...
- [x] **Step 5: Run context, CLI, and all tests**
...
- [x] **Step 6: Commit V2 output and docs**
...
- [x] **Step 1: Confirm the evaluation checkout is the pinned Click commit**

Run: `git -C /tmp/ai-repo-doctor-v2-click rev-parse HEAD`

Expected: `06b2a678741131fd577ce170e23e5ca0aeba0309`.

- [x] **Step 2: Scan Click and assert the representative relationships**
...
- [x] **Step 3: Verify the known dynamic-dispatch boundary stays unresolved**

Inspect `src/click/core.py::Group.invoke` call sites. For each call record whose caller is `src/click/core.py::Group.invoke` and expression is `sub_ctx.command.invoke`, assert there is no call edge with the same caller and line targeting `src/click/core.py::Command.invoke`. `call_edges` stores caller, callee, and line but not the original expression, so another call on the same line may resolve independently: at line 2082, the outer `_process_result(...)` call correctly resolves while the nested dynamic dispatch stays unresolved. Confirm both `sub_ctx.command.invoke` expressions remain in `calls` and no matching `Command.invoke` edge exists at either site; do not reject unrelated same-line call edges.

- [x] **Step 4: Rerun the Requests regression smoke**
...
- [x] **Step 6: Review the final V2 diff and commit any narrowly scoped evaluation correction**
...
- [x] **Step 7: Push the reviewed V2 branch**
...
- [x] Re-export resolution has explicit positive and negative tests, including cycles and conflicts.
- [x] Overload handling selects only one concrete implementation and preserves ambiguity otherwise.
- [x] Click registration edges use decorator line evidence and never count as call edges.
- [x] Context source budgets apply to semantic neighbors; impact keeps semantic relationships outside call depth.
- [x] All command JSON outputs use schema version 2 and V1 field meanings remain stable.
- [x] Click evaluation checks the pinned commit, five callbacks, `click.echo`, overload cases, and the unresolved dynamic-dispatch boundary.
