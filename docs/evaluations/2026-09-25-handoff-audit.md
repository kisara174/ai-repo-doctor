...
| M06 | Closed after one failed attempt | One DeepSeek transport attempt used a clean but incorrect Requests checkout at `0b401c76b6e80a4eecf3c690085b2553f6e261ca`, instead of the manifest-pinned `7a13c041dbef42f9f3feb14110f02626f6892e9a`. It returned a connection error with no valid response or usage record; billing is unknown. No retry was made. Details: `docs/evaluations/2026-09-25-live-smoke.md`. |
| M07 | Blocked | M06 produced no valid model response, so there are no manual finding labels and no model-quality claims. Details and reconsideration gate: `docs/superpowers/specs/2026-09-25-next-iteration-candidates.md`. |
| M08 | Measurement complete; no optimization implemented | Five local preparation runs measured a 5.4590 s median total, with `build_index` at 5.2382 s (~96%) and two redundant builds for repeated snapshots. No code or API behavior changed. Details: `docs/evaluations/2026-09-25-preparation-performance.md`. |
| M09 | Candidate planning complete | Ranked a fresh one-call connectivity experiment (requiring new explicit authorization), invocation-local index reuse, and deferring additional bug/fix pairs until valid human-reviewed model labels exist. Details: `docs/superpowers/specs/2026-09-25-next-iteration-candidates.md`. |
| M10 | Index reuse implemented and locally verified; PR CI pending publication | Commit `ad07876172ea5af6e995cd7ac1765c97110dde17` caches indexes only within `prepare_cases`, keyed by `(checkout_id, commit)`. The focused regression demonstrates one build per snapshot per invocation while fingerprints still run for every case. All 10 M05 case records and contexts match; index builds fell from 10 to 8. Full suite: 256 tests; compileall, CLI help, and diff checks passed. Five-run timings and limitations: `docs/evaluations/2026-09-25-preparation-cache.md`. |

Luna executor dispatch was attempted for both the audit update and the bounded index-cache change, but the platform returned `agent thread limit reached`; the primary agent implemented and reviewed the cache and documented the results. PR #7's remote head is still `24fb68ce67297c80d0c643e6b87802b2e6b009cb`, with all six CI jobs passing; local follow-up commit `ad07876172ea5af6e995cd7ac1765c97110dde17` and its evaluation report are not yet pushed. M06's single attempt is closed with the documented connection failure, M07 remains blocked, and M08–M10 offline work is complete. PR #7 remains open and unmerged; do not merge it without authorization for that PR's merge.
