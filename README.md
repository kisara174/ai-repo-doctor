...
- 调用图只连向静态可定位的局部目标。对局部变量，只解析唯一的简单本地类构造绑定（例如 `client = Client()` 或 `with modules.Client() as client`）；复杂工厂、重绑定和多态分派仍可能无法解析。重名的条件定义会标为歧义；函数内部 import 的别名、动态 import、反射、猴子补丁、别名传播和外部包调用也可能无法解析。关联测试仅表示静态引用，不等于测试覆盖率。
